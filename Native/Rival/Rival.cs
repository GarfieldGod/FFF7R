...
//         List<List<int>> RivalViewChessStatus = Utils.DeepCopy(chessPad.StatusMap);
...
//             result = new Input(chessGridPos, bestResultInPosEffect.Item2.Property, PlayerType.RIVAL);
//         }
//         return result;
//     }
...
//                 List<List<int>> chessGridStatusTemp = PosEffect.DoPosEffect(chessGridPos, property.PosEffect, chessGridPosStatus, PlayerType.RIVAL);
...
//         //             if (buff.scope == EffectTarget.FriendOnly) buff.scope = EffectTarget.EnemyOnly;
//         //             else if (buff.scope == EffectTarget.EnemyOnly) buff.scope = EffectTarget.FriendOnly;
//         //             buffs[z] = buff;
//         //         }
//         //     }
//         // }
...
//         List<List<int>> result = Utils.DeepCopy(chessPosStatus);
...
//         List<List<int>> srcTemp = Utils.DeepCopy(src);
//         foreach(var line in srcTemp) {
//             int left = 0;
//             int right = line.Count - 1;
//             while (left < right)
...
//         HashSet<Int2D> result = new HashSet<Int2D>();
//         for(int i = 0; i < chessStatus.Count; i++) {
//             for(int j = 0; j < chessStatus[i].Count; j++) {
//                 if (playerType == PlayerType.PLAYER)
//                 {
